...
- 启动脚本：`start.ps1`
  ...
  - 一次到位（推荐顺序）：
    ...
    docker compose -f orchestration/infra/docker-compose.infra.yml up -d postgres redis
...
- 快速启动：
  ...
  # 启动 RagFlow（会使用模板生成 service_conf.yaml）
...
- 参数：
  - `-IncludeModules`：除基础服务外，同时清理以 `ai-` 开头的功能模块容器（如 `ai-oneapi`）
  - `-Force`：跳过二次确认
- 编码/执行策略：
