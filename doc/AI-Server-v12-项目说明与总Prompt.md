...
- 前端：
  - 使用 `ui-ai-server-v12` 作为唯一 UI 基线，保留其项目结构（可在内部重构）。
- 后端 / 主进程：
...
1. **模块注册表**：
   - 你需要设计一个**可扩展**的数据结构（例如一个 JSON/TS 文件），描述每个模块，保证未来新增模块（例如 Ollama Web 端）时只需要新增配置、少量 UI 挂载即可完成集成：
     - `id`：内部唯一标识，用于路由、日志归类等；
     - `name`：展示给用户的模块名，如 `n8n`、`dify`、`oneapi`、`ragflow`、`ollama-web` 等；
     - `type`：`feature`（功能模块）或 `basic`（基础服务）；
     - `dependsOn`：依赖的基础服务列表；
     - `ports`：默认端口映射（containerPort -> hostPort），以及是否允许用户配置；
     - `healthCheck`：健康检查方式（如 HTTP 探测地址和超时时间）；
     - `composeTemplate`：docker-compose 模板路径或内联片段（也可以支持“仅外部 URL 模块”，如纯 Web SaaS，而不走 Docker）；
     - 其它必要的运行参数（环境变量、卷挂载路径等）。
   - 尽量让“新增模块”的工作流是：**新增一条注册表记录 +（可选）新增一个 Docker Compose 模板 + 前端侧挂一个菜单/路由**，避免到处硬编码模块名称。

2. **启动模块**：
   - `startModule(name)` 的典型流程：
     - 检查 Docker/compose 是否就绪；
     - 解析模块及其依赖，按依赖顺序启动：
       - 对于尚未创建的容器：执行 `docker compose up -d`；
       - 对于已存在但已停止的容器：执行 `docker start`；
     - 检查端口冲突：宿主机端口是否已被其它进程占用；
     - 对每个模块执行健康检查（如不断请求某个 HTTP 接口直至成功或超时）；
     - 将过程中的关键事件记录到操作日志中（供前端查看）。

3. **停止模块**：
   - `stopModule(name)`：
     - 允许安全停止 feature 模块；
     - 对基础服务，需要考虑是否还有其它 feature 正在使用，避免误停导致连锁中断；
     - 提供配置项决定是否自动停止“不再被任何模块使用的基础服务”。

4. **模块状态**：
   - `getModuleStatus(name)` 需要返回：
     - 当前是否有相关容器在运行；
     - 聚合后的状态（如 `running` / `stopped` / `error` / `starting` 等）；
     - 当前映射的宿主机端口；
   - 前端会周期性轮询并在 Dashboard 上更新卡片信息。

5. **清理能力**：
   - 提供若干操作（挂在 Settings/调试页的“危险区域”）：
     - 停止所有相关容器；
     - 删除相关容器；
     - 删除相关卷；
     - 删除自定义网络；
...
1. **容器日志**：
   - 从 Docker 获取指定容器或模块的最近 N 行日志；
   - 可以通过 `docker logs` 命令或库的方式实现；
   - 标准化为统一的结构：时间戳、级别、模块名、容器名、消息内容。

2. **应用操作日志（Ops Log）**：
   - 每次关键操作（如启动/停止模块、执行危险清理操作）都写一条操作日志；
   - 可以写入本地文件（滚动日志），并保留一定内存缓存；
   - 提供接口给前端查询最近 N 条操作日志。
...
3. **前端 Logs 页面**：
   - 支持：
     ...
     - 分页或虚拟滚动展示大量日志；
   - 点击某一条日志可以快速筛选同模块/同服务相关的日志（可选）。

### 3.4 BrowserView 嵌入模块 Web 页

1. **模块类型**：
   - n8n、Dify、OneAPI、RagFlow：本机容器提供的 Web 服务；
   - 在线教程、AI 市场：外部 URL（可在配置中维护）。

2. **功能需求**：
   - 通过 Electron 的 BrowserView 或等价机制：
     - 在主窗口中，为每个模块/页面创建对应的 Web 视图；
     - 根据当前路由/选中模块，显示或隐藏不同的 BrowserView；
   - 工具栏：
     - 后退 / 前进；
     - 刷新；
     - 回到模块首页（预设的 Home URL）；
...
3. **布局与尺寸**：
   - 需要与 v12 UI 的布局配合：
     - 顶部自定义标题栏/导航条；
     - 左侧菜单（如有）；
     - 中间内容区中，BrowserView 只占用主内容区域，不遮挡顶部/侧边栏；
   - 需要在窗口 resize / 最大化 / 还原时，动态调整 BrowserView 的 bounds。
...
1. **系统设置**：
   - 应用名称；
   - UI 语言（至少保证中文可用，预留英文）；
   - 新窗口打开方式（在应用内打开 / 系统浏览器打开）；
   - 日志级别（影响 Ops Log 和控制台输出）。

2. **网络设置**：
   - Docker 镜像加速地址列表；
   - 代理模式（直连/系统代理/手动），手动模式下的主机 + 端口；
   - 说明这些设置如何影响 Docker 拉取镜像（可以做名字重写、提醒用户配置 Docker Desktop 的 registry-mirrors/代理等）。
...
3. **模块设置**：
   - 每个模块可独立配置：
     ...
     - 数据库连接串；
     - 额外环境变量（以文本或 KV 形式）；
   - 需要有合理的持久化格式（如 JSON/YAML），并在启动模块前生效。

4. **调试与危险操作**：
   - 打开前端 DevTools；
   - 打开特定模块 BrowserView 的 DevTools；
   - 清空某个模块在本地的存储数据（cookies/localStorage/indexedDB 等）；
   - 执行前面提到的 Docker 清理操作（停止/删除容器、删除卷/网络、一键核弹清理）。

### 3.6 监控页面

- 第一阶段：
  - 保持 v12 UI 中已有的监控卡片和“伪折线图”结构；
  - 使用本地假数据 + 定时器模拟变化；

- 后续阶段（可选）：
  - 接入 Docker 统计或系统监控数据（如 CPU/内存/磁盘使用率、各模块响应时间等）。

### 3.7 打包与分发

- 设计 `package.json` 与 electron-builder 配置：
  - `npm run dev`：启动 React 前端 + Electron 主进程，进入开发模式；
  - `npm run build`：构建前端静态资源与 Electron 主进程；
  - `npm run dist`：执行 electron-builder 打包，输出 Windows 安装包；
- 注意：
  - 打包产物中包含：主进程 JS/TS 编译结果、前端打包后的静态文件、Docker 配置模板、必要的脚本与配置文件；
  - 安装后，普通用户无需安装 Node.js/npm，就能运行客户端（仍需手动安装 Docker Desktop）。
...
2. **阶段 1：初始化 Electron + React 工程骨架**
   ...
   - 编写最小化 Electron 主进程入口、创建窗口、加载 v12 UI（开发模式下可走 Vite dev server）；
   - 为后续 IPC 通信预留 preload 文件；
   - 新增 npm 脚本，说明如何在开发环境启动应用。
