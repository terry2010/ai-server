...
interface LogEntry { id: number; timestamp: string; level: 'error' | 'warn' | 'info' | 'debug'; service: string; message: string }
...
const generateMockLogs = () => {
  ...
  const messages = ['服务启动成功', '数据库连接建立', '用户认证失败', '内存使用率过高', 'API 请求处理完成', '配置文件加载失败', '定时任务执行', '缓存清理完成', '网络连接超时', '文件上传成功']
  const mockLogs: LogEntry[] = []
  for (let i = 0; i < 50; i++) {
    const now = new Date(); now.setMinutes(now.getMinutes() - i * 2)
    mockLogs.push({ id: i + 1, timestamp: now.toLocaleString('zh-CN'), level: levels[Math.floor(Math.random() * levels.length)], service: services[Math.floor(Math.random() * services.length)], message: messages[Math.floor(Math.random() * messages.length)] })
  }
...
const filterLogs = () => { message.info(`已筛选 ${logLevel.value} 级别日志`) }
const clearLogs = () => { logs.value = []; message.success('日志已清空') }
const refreshLogs = () => { logs.value = generateMockLogs(); message.success('日志已刷新') }

onMounted(() => { logs.value = generateMockLogs() })
</script>
...
.log-container { height: calc(100vh - 200px); overflow-y: auto; font-family: var(--font-mono); font-size: var(--text-sm); }
.log-entry { display: grid; grid-template-columns: 180px 80px 100px 1fr; gap: var(--spacing-md); padding: var(--spacing-sm) var(--spacing-md); border-bottom: 1px solid var(--border-light); transition: background-color var(--transition-base); }
