...
function Remove-NetworkIfExists($name) {
  ...
  if ($exists) {
    # check if network is still in use by containers other than ai-mysql
    ...
    if ($raw -and $raw -ne 'null') {
      try {
        ...
        if ($values.Count -gt 0) {
          ...
          $others = $names | Where-Object { $_ -ne 'ai-mysql' }
          if ($others -and $others.Count -gt 0) { $inUseByOthers = $true }
        }
      } catch {}
    }
    if ($inUseByOthers) {
      Write-Host "[mysql] network in use by other containers, skip: $name"
    } else {
      docker network rm $name | Out-Null
      Write-Host "[mysql] removed network: $name"
    }
  }
}
...
if (-not $Force) {
  $ans = Read-Host "Confirm to clear MySQL resources? (containers/volumes/networks, images will NOT be removed) (y/N)"
...
Write-Host '[mysql] cleanup done (no images were removed)'
exit 0
