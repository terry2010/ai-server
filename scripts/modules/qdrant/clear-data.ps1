    ...
    if ($raw -and $raw -ne 'null') {
      try {
        ...
        if ($values.Count -gt 0) {
          ...
          $others = $names | Where-Object { $_ -ne 'ai-qdrant' }
          if ($others -and $others.Count -gt 0) { $inUseByOthers = $true }
        }
      } catch {}
    }
    if ($inUseByOthers) {
      Write-Host "[qdrant] network in use by other containers, skip: $name"
    } else {
      docker network rm $name | Out-Null
      Write-Host "[qdrant] removed network: $name"
    }
  }
}
...
if (-not $Force) {
  $ans = Read-Host "Confirm to clear Qdrant resources? (containers/volumes/networks, images will NOT be removed) (y/N)"
  if ($ans -ne 'y' -and $ans -ne 'Y') { Write-Host 'Canceled'; exit 0 }
}

$doAll = $All -or (-not $Containers -and -not $Volumes -and -not $Networks)
if ($doAll -or $Containers) { Remove-ContainerIfExists 'ai-qdrant' }
if ($doAll -or $Volumes) { Remove-VolumeIfExists 'ai-server-qdrant-data' }
if ($doAll -or $Networks) { Remove-NetworkIfExists 'ai-server-net' }

Write-Host '[qdrant] cleanup done (no images were removed)'
exit 0
